...
7. **Lifespan.** Experiment, personal tool, or maintained app; the answers can combine, and a personal tool you keep using is also maintained. If pure experiment, skip most of the ceremony in this kit and just build.
8. **Risk.** Three separate questions, each answered on its own: is it internet-exposed (a public URL counts, even if only you ever open it), is it multi-user, and does it handle sensitive data. Ceremony scales with risk, not lifespan: internet-exposed means the security checklist runs before any URL is shared, multi-user means auth gets tested as someone other than you, sensitive data means the data rules are filled in before the first real record is stored. A weekend experiment that touches real customer data is still high risk.
